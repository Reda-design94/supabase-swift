...
//
  ...
  // `_clock` (Sources/Helpers/_Clock.swift) is a process-global mutable seam swapped to a
  // `TestClock` below so reconnect-delay tests don't wait in real time. Serialize this suite so
  // its tests don't race each other's clock swaps — Swift Testing runs `@Test`s in the same
  // suite concurrently by default, unlike XCTest's implicit one-class-at-a-time execution. Use a
  // class (not a struct) so `deinit` can disconnect both clients afterward, mirroring the old
  // `tearDown()`.
  @Suite(.serialized)
